...
**I1 is on `require_analyst_approval_checkpoints` by default.** A session-level instruction such as "kör på defaultvärden utan frågor" / "run with defaults" does NOT bypass I1 — that phrase only overrides questions about data, methodology, language, and scope. Composing archetypes is an analyst-owned analytical decision, so the skill halts here by default.

**Exception — `mode: auto`.** If `state.json.mode == "auto"` (or `.persona-config.md` sets `process.mode: auto`), do **not** halt. Keep the archetype line-up as drafted, append an I1 entry to the auto-decision log (`09-auto/auto-decisions.md`) recording the archetype list, the `[inferred]`-flag counts, the orphan-material count, the top-3 audit risks, and the `verify-quotes.py` worst-verdict, print one `AUTO ▸ generate-archetypes:I1 — kept N archetypes as drafted` line, then update `state.json` `status: completed` (the `run-pipeline` orchestrator advances to `validate-archetypes`). **Note the asymmetry:** the quote-verifier (below) must still pass before I1 even in auto mode — auto mode skips the human pause, never the falsification. This is the deliberate, explicit authorisation in `${CLAUDE_PLUGIN_ROOT}/references/cross-cutting-principles.md` § "`mode` (autonomous run)". The other explicit bypass is `process.skip-checkpoints: yes` (synthetic / test run — emits a loud warning and marks the run synthetic). See also § "What `strict-checkpoints: yes` is **not** overridden by".
