...
- **Open CRITICAL gating (H22-fix).** Read `state.json.steps.validate-archetypes.open_critical_findings`. If the list is non-empty, **HALT** at the Read beat and surface to the analyst:
   ...
   Do **not** proceed past Read until the analyst signals explicit override or resolves the findings. A silent "warn and proceed" is not enough — that's how H22 leaked through on SJ Återförsäljare 2026-05-28.
...
**D0 is on `require_analyst_approval_checkpoints` by default.** A session-level instruction such as "kör på defaultvärden utan frågor" / "run with defaults" does NOT bypass D0 — image aesthetic (#2, marked "THE MOST CRITICAL") and per-persona character briefs (#4) cannot be auto-resolved without an analyst signal. The skill MUST surface all eight defaults and wait for explicit confirmation even under auto-mode. The only way to bypass is `process.skip-checkpoints: yes` in `.persona-config.md`, which emits a loud warning. See `${CLAUDE_PLUGIN_ROOT}/references/cross-cutting-principles.md` § "What `strict-checkpoints: yes` is **not** overridden by".

If `skip-checkpoints: yes` is set and D0 is bypassed, the rendered output must still carry a banner *"Pre-design briefing skipped — defaults applied"* on the inline preview, and the `06-design/README.md` (if emitted) must note which defaults were applied without analyst review.
